...
Repo: **`RJiazhen/learn-by-diff`** (default branch `main`).

Detect phase → read **only** that phase file → execute → stop. Never chain phases in one call. Never load all phase files at once.

## User-facing output

Phase files and this skill’s tables are **agent-only**. Do not paste, quote, or restate them in chat. Reply with a brief result plus the response template only.
...
| ------------------------------------- | ----------- | -------------------------------------------------- |
| On `main`, no feature-branch context  | **Start**   | [phases/start.md](./phases/start.md)               |
| Feature branch, acceptance incomplete | **Develop** | [phases/develop-wait.md](./phases/develop-wait.md) |
| Dirty worktree / index                | **Commit**  | [phases/commit.md](./phases/commit.md)             |
| Unpushed commits or no PR yet         | **Ship**    | [phases/ship.md](./phases/ship.md)                 |
| OPEN PR, clean, synced with origin    | **Merge**   | [phases/merge.md](./phases/merge.md)               |
| Pushed, PR open, waiting              | **Wait**    | [phases/develop-wait.md](./phases/develop-wait.md) |
