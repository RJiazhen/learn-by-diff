...
| 操作                       | 内容                                       |
| -------------------------- | ------------------------------------------ |
| **前の章 / 次の章**        | 隣接する章の開始スナップショットへ切り替え |
| **章を検索**               | タイトルまたは ID で章を探して一覧に表示   |
| **章のドキュメントを開く** | その章のドキュメントを開く                 |
| **未開始**                 | その章の開始スナップショットを適用         |
| **完了**                   | その章の完了スナップショットを適用         |

コードはいつでも編集できます。章を切り替えると、**現在のフォルダーを上書きするか**確認されます。`.gitignore` で無視されているトップレベルのファイルやフォルダー（例: `node_modules/`）は残されます。詳しくは [残すファイル](../course-config/retained) を参照してください。
...
異なる章・状態のコードを並べて動かしたいときは、章名の右にある **「未開始フォルダーを開く」** または **「完了フォルダーを開く」** をクリックします。そのスナップショットがダウンロードされ、ワークスペースに別フォルダーとして追加されます。
